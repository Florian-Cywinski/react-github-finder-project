...
function User() {
  ...
  useEffect(() => {
    ...
    const getUserData = async () => {
      const userData = await getUserAndRepos(params.login)        // getUser comes from GithubActions.jsx
  ...
  const {
    ...
    public_gists,
    hireable,
  } = user  // Destructuring from this user object (GithubContext)
          ...
          <div className='col-span-2'>
            <div className='mb-6'>
              <h1 className='text-3xl card-title'>{name}
                ...
                {hireable && (  // It's shown if it was set by the wanted user
            ...
            <div className='w-full rounded-lg shadow-md bg-base-100 stats'>
              {location && (  // It's shown if it was set by the wanted user
                <div className='stat'>
                  <div className='stat-title text-md'>Location</div>
                  <div className='text-lg stat-value'>{location}</div>
                </div>
              )}
              {blog && (      // It's shown if it was set by the wanted user - blog is the website of the wanted user
                <div className='stat'>
              ...
              {twitter_username && (  // It's shown if it was set by the wanted user
                <div className='stat'>
